...
export default function Card({ api, title }) {
    return (
        <div className="lg:max-w-6xl md:w-auto md:max-w-3xl w-[300px] mt-28 -mb-28 md:-mb-8">
            <h1 className="md:text-4xl mb-4 md:mb-8 font-bold underline text-gray-700">{title}</h1>
            <div className="w-full z-20 flex flex-wrap justify-between">
                {api.map((data) => {
                    return (
                        <div key={`${data.mal_id}/${title}`} className="h-48 w-[74px] md:h-96 md:w-52 lg:w-64 pb-16 lg:pb-32 hover:scale-[102%]">
